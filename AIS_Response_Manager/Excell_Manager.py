...
FileRoute_xlsx = 'AIS-Responder_Data.xlsx'
...
def ReadExcell():
    df = pd.read_excel(FileRoute_xlsx) 
    print(df)
...
def AddRowExcel():
    ...
    if os.path.exists(FileRoute_xlsx):
        df_existing = pd.read_excel(FileRoute_xlsx)
    else:
    ...
    # Save
    df_combined.to_excel(FileRoute_xlsx, index=False)
...
def DropRow(_row):
    ...
    if (os.path.exists(FileRoute_xlsx) == False):
    ...
    # Save updated file
    content.to_excel(FileRoute_xlsx, index=False)
