...
class RadarDrawing:
    ...
    def plot_otherBoat(self, myBoat, boatToSave, x_meters, y_meters, number, scale=0.050, heading=0.00, IsTarget = False):
        ...
        # config offset is the amount offset the verious x and y combinations will try it should be equal or greater than min_range to minimize colision risks with the same id_loc
        ...
        id_draw, px = self._isNumLocXValid(px, py, min_range, -offset, id_draw)
        
        # try px + offset offset
        id_draw, px = self._isNumLocXValid(px, py, min_range, offset, id_draw)

        # try py - offset offset
        id_draw, py = self._isNumLocYValid(px, py, min_range, -offset, id_draw)
    ...
    def _isNumLocXValid(self, px, py, min_range, offset, id_draw):
        if id_draw == False:
            id_draw = True
            mod_px = px - offset
            for loc in self.id_locs:
    ...
    def _isNumLocYValid(self, px, py, min_range, offset, id_draw):
        if id_draw == False:
            id_draw = True
            mod_py = py - offset
            for loc in self.id_locs:
